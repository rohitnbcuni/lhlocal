		...
		public function buildQCProjectCCListHTML($proj_id, $ccList){
								...
								<div class="side_bucket_container bucket_container_last">
									...
									<div class="side_bucket_content">';
										...
										<div class="cclist_actions" id="select_cc" style="display: none;">
											<select name="qccc_user" id="qccc_user" style="margin-bottom:2px;width:341px">';
												if(isset($proj_id)) {
													echo AdminDisplay::getUserOptionEditHTML();
												}
											echo '</select>
...
//////////////////////////////////
	...
	function updatecompanyAction(){
		...
		function insertcompanyAction(){
			$comnpany_array = array();
			$comnpany_array['name'] = $this->_request->getParam('company_name');
			if(!empty($comnpany_array['name'])){
				$db = Zend_Registry::get('db');
				$adminDisplay = new AdminDisplay();
				$exist_comapny = $adminDisplay->getCompanyDetailsByName($comnpany_array['name']);
				if($exist_comapny == true){
					echo "Exist";
