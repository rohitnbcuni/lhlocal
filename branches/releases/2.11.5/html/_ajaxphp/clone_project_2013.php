<?PHP
	...
	//global $mysql;
	$project_all = "SELECT * FROM `projects` WHERE `archived` = '0' AND `active` = '1' AND `deleted` = '0' AND year='2012' AND company IN ('64i')";
	...
	while($row = $project_list->fetch_assoc()) {
		...
		{  // echo $i++;
			...
			//echo "<br/>";
			$resources = $mysql->query($project_resources_2012);
			...
			$archiveCloneProject = "UPDATE `projects` set `archived` = '1', project_code='".$row['project_code'].'_2012'."' WHERE ID = '".$clone_project_id."'";
