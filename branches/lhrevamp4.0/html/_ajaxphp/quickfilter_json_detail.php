...
//	$select_project_phases = "SELECT * FROM `project_phase_finance` WHERE `project_id` = '" .@$mysql->real_escape_string($_GET['projID']) ."'";
	...
	while($row_phases = $result_phases->fetch_assoc()) {
		...
		$timeline_query = "SELECT * FROM `project_phases` WHERE `project_id`='" .$mysql->real_escape_string($_GET['projID'])."' AND `phase_type`='" .$row_phases['phase'] ."' order by id DESC LIMIT 1";
