	...
	for($d=1;$d<=$numberOfDays;$d++){
		...
		if($currentDay < 6) {
			if($d == 1 && $firstMonthDay < 7) {
				$sel_class = 'sel';
				$open = true;
				$sqlOt = "SELECT * FROM `resource_blocks` WHERE `userid`=? AND `datestamp` = ? AND `daypart` = ? AND `hours` > ?";
				$resOt = $mysql->sqlprepare($sqlOt,array($userid, date('Y/n/j', mktime(0, 0, 0, $currentMonth, $d, $currentYear)), 9,0) );
				if($resOt->num_rows > 0) {
					$otClass = "cancel";
				} else {
					$otClass = "overtime";
				...
				$html .= '<!--== | START : Day ==-->
					<li class="schedule_day">
						...
						for($it = 0; $it < 8; $it++) {
							...
							$sqlrp = "SELECT * FROM `resource_blocks` a LEFT JOIN `projects` b ON a.`projectid`=b.`id`  WHERE a.`userid`=? AND a.`datestamp` = ? AND a.`daypart` = ?";
							$resrp = $mysql->sqlprepare($sqlrp,array($userid,$dayDate,($it+1)));
