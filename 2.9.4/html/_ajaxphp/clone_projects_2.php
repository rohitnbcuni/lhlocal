<?PHP
	...
	while($row = $project_list->fetch_assoc()) {
		...
		{  // echo $i++;
			...
			$proj_brief_sec_sql = $mysql->query($proj_brief_sec);
			if($proj_brief_sec_sql->num_rows > 0){
				while($rows = $proj_brief_sec_sql->fetch_assoc()) {
					...
					$mysql->query($proj_brief_new_sec);
				}
			}
			...
			$insert_budget_sql = $mysql->query($insert_budget);
			...
			$project_phase_sql = $mysql->query($project_phase);
			...
			$project_phase_approvals_sql = $mysql->query($project_phase_approvals);
			if($project_phase_approvals_sql->num_rows > 0){
				while($rows = @$project_phase_approvals_sql->fetch_assoc()) {
					...
					$mysql->query($project_phase_new_approvals);
				}
			}
			...
			$project_phase_finance_sql = $mysql->query($project_phase_finance);
			if($project_phase_finance_sql->num_rows > 0){
				while($rows = @$project_phase_finance_sql->fetch_assoc()) {
					...
					$mysql->query($project_phase_new_finance);
				}
			}
			...
			$project_risks_sql = $mysql->query($project_risks);
			...
			$project_sub_phase_finance_sql = $mysql->query($project_sub_phase_finance);
			if($project_sub_phase_finance_sql->num_rows > 0){
				while($rows = $project_sub_phase_finance_sql->fetch_assoc()) {
					...
					$mysql->query($project_sub_phase_finance_new);
			...
			$qa_project_version_sql = $mysql->query($project_sub_phase_finance);
			...
			$mysql->query($qa_project_version);
			//End qa_project_version
			
			//resource_blocks
		    $project_sub_phase_role  = 	"SELECT * FROM `resource_blocks`  WHERE `projectid` = '".$clone_project_id."'";
		    $project_sub_phase_role_sql = $mysql->query($project_sub_phase_role);
			...
			$archiveCloneProject = "UPDATE `projects` set `archived` = '1', project_code='".$row['project_code'].'_2011'."' WHERE ID = '".$clone_project_id."'";
			...
			$deleteoldProject2010 = "update `projects` SET `deleted`='1' where `YEAR`='2010'";
			$mysql->query($deleteoldProject2010);
			$deleteoldProject2011 = "update `projects` SET `deleted`='1' where `YEAR`='2012' and archived='1'";
			$mysql->query($deleteoldProject2011);
