...
function ctCalender(){
	...
	var producerID =  $('#producer_filter').val();
	var quarterID =  $('#quarter_filter').val();
	var programID =  $('#program_filter').val();
	var statusID = $('#approval_filter').val();
	var projectTitle = $('#project_filter').val()
	$("#company_filter option[@value=-1]").remove();
	var query = 'companyID='+companyID+'&ProgramID='+programID+'&producerID='+producerID+'&quarterID='+quarterID+'&statusID='+statusID+'&projectTitle='+projectTitle;
	...
	if(controllerName == 'calendarview'){
		...
		$.ajax({
			type: "POST",
			url: "/_ajaxphp/ct_calendarview_all.php",
			data: "date="+selDate.getFullYear()+"/"+(selDate.getMonth()+1)+"/"+selDate.getDate()+"&"+query,
			success: function(msg){
			$('#wo_dimmer_ajax_cal').css({display:'none'});	
			$('#ct_calender_view').html(msg+"<br/>");
			}
		});
