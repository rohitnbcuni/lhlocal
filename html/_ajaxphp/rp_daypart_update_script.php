...
global $mysql;
  ...
  if($daypart == "4"){	
	  ...
	  $overtime_daypart = $mysql->sqlordie($overtime_daypart_query);	
  }
  ...
  $all_daypart = $mysql->sqlordie($all_daypart_query);
  ...
  if(@$all_daypart->num_rows > 0) {
    ...
    while($row = @$all_daypart->fetch_assoc()) {
      ...
      $mysql->sqlordie($update_daypart_query);
      ...
      $mysql->sqlordie($insert_daypart_query);
