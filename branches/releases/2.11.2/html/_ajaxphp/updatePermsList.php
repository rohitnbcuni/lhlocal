<?PHP
	...
	$default = $mysql->query("SELECT a.`id`,a.`name` FROM `companies` a, `projects` b WHERE b.`id`='$projectId' AND a.`id`=b.`company` LIMIT 1");
	...
	if(!empty($projectId)) {
		...
		for($i = 0; $i < sizeof($comp_keys); $i++) {
			...
			$res_user_list = $mysql->query($get_user_list);
