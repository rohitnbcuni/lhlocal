...
class Workorders_IndexController extends Zend_Controller_Action
{
		...
		
	{
		...
		$this->cache = Zend_Cache::factory('Core','File',array('caching' => false, 'lifetime' => 7200, 'automatic_serialization' => true), array('cache_dir'=>"/tmp/","file_name_prefix"=>session_id()));
		...
		$this->name = sprintf("%s %s", $this->person->{"first-name"}, $this->person->{"last-name"});
	}
	...
	public function load($var)
	{
		...
		{	
				...
				{
					$this->$var = $this->{$this->function_name}();
	...
	public function getPerson()
	{
		...
		foreach($this->companies as $key => $val) 
		{
			...
			if(is_array($people)) 
			{
				...
				{
					if(isset($person->{"user-name"}) && (strtoupper($person->{"user-name"}) == strtoupper($this->username)))
					{
						return $person;
					}
				}
	...
	public function getCompanies()
	{
		...
		{
			$companies[$project->company->id] = array("id" => $project->company->id, "name" => $project->company->name);			
		}
	...
	public function getWorkorderCategory($project)
	{
			...
			print "Error: You need a category in Basecamp called Workorder";die;
	}
 ...
 
{
    ...
    public function listAction()
    {
		...
		$select = $db->select()->from("v_workorders");
			 ...
			 if(!empty($local)) $list[$project->company->id][] = array("company"=>$project->company->name,"project" => $project->name, "id" => $project->id, "status" => $project->status, "workorders" => $local);

		}
		
		foreach($this->resources as $person)
		{        
			$options2[$person->id] = $person->{'last-name'} .", ". $person->{'first-name'}; 
		}
...
;  
                ...
                $update = array('category_id' => $row->category_id);                
               
                $notify = array($resource->id, $this->que_mgr->id, $row->requested_by);
           ...
           $id = (int)$this->_request->getParam('id', 0);
            if ($id > 0) {
				...
				$form->project_id->setMultiOptions(array($workorder->project_id => $options1[$workorder->project_id]));
				$form->project_id->setValue($workorder->project_id);
				$requester = $this->bc->person($workorder->requested_by);
				$requester->name = sprintf("%s %s", $requester->{"first-name"},$requester->{"last-name"});
				$comments = $this->getComments($workorder->bcid);
				$form->requested_by->setMultiOptions(array($workorder->requested_by => $requester->name));
   ...
   public function createAction()
    {
		...
		$form = new WorkorderForm();  
		$form->setDecorators(array(array('viewScript', array('viewScript' => 'index/create.phtml')))); 		
        ...
        if ($this->_request->isPost()) {
            ...
            if ($form->isValid($formData)) {
                ...
                'title' => (!strpos("WO:",$formData['title']) ? "WO: " : "") . $formData['title'] . ' [' . $formData['priority'] . ']',
                'body' => $formData['body'], 
                'file' => $_FILES['file']['name'],               
                'requested_by' => $formData['requested_by'],
                'assigned_to' => $formData['assigned_to'],
                'extra_resources' => $formData['extra_resources'],
                'start_date' => $date->getIso(),
                'created_date' => $date->getIso(),
                'modified_date' => $date->getIso(),
                'created_by' => $this->bc->user,
                'modified_by' => $this->bc->user                
                );
				...
				preg_match('/\<upload\>\<id\>([0-9A-Za-z.]+)\<\/id\>\<\/upload\>/',$response->getBody(), $found);
					...
					$attachments = 
						array(
							...
							'file' => 
								array(
										'file' => $found[1], 'content-type' => $_FILES['file']['type'], 
										'original-filename' => $_FILES['file']['name']
								)
						);
					
                $notify = array($this->person->id,$this->que_mgr->id);
		...
		{        
			$options2[$person->id] = $person->{'last-name'} .", ". $person->{'first-name'}; 
			if($this->bc->user == $person->{'user-name'}) $this->person =  $person;    	
		}
		...
		$form->extra_resources->setMultiOptions($wodb->getEnums('extra_resources'));
		$form->status->setMultiOptions($wodb->getEnums('status'));	
		$form->requested_by->setMultiOptions(array($this->person->id => $this->name));	
 ...
 
{
		...
		$wodb = new Workorders();
            ...
            if (!$this->_request->isPost() && false) {
				...
				$workorder->{"closed_date"} = date('Y-m-d');
				$workorder->save();
...
public function commentAction()
{
	    ...
	    $body = $formdata['new_comment'];
			...
			foreach($comments as $comment) {
				...
				$comment->{"posted"} = $comment->{"posted-on"};
				$comment->{"attachments"} = $comment->{"attachments-count"};
				//javascript will parf on these property names...
				unset ($comment->{"author-id"});					
				unset ($comment->{"first-name"});	
				unset ($comment->{"last-name-name"});							
				unset ($comment->{"posted-on"});			
				unset ($comment->{"attachments-count"});
			}			
	...
	$this->getProjectLOV();
		...
		foreach($this->projects as $project)
		{
			 ...
			 $temp = "";
			 $wodb = $db->query("SELECT * from v_workorders where project_id  = ?", array($project->id));
			 $db->setFetchMode(Zend_Db::FETCH_OBJ);
			 $local = $wodb->fetchAll();
