			...
			if ((clientId < 0 || clientId == qualityList[i]['client']) && (projectId < 0 || projectId == qualityList[i]['project_id'])) {
				...
				for (var e = 0; e < qualityList[i]['quality'].length; e++) {
					...
					else
					{
						...
						if(qualityList[i]['quality'][e]['version'].length != '' && qualityList[i]['quality'][e]['version'].length > 16){
							var version_truncate = qualityList[i]['quality'][e]['version'].slice(0,10)+"...";
						}else{
							var version_truncate = qualityList[i]['quality'][e]['version'];
						}
						html_body += '<dd class="version" title="'+ qualityList[i]['quality'][e]['version']+'">' +version_truncate+ '</dd>';
...
function generateWOReport(){
	...
	var projectId =  $("#project_filter").val();
	...
	window.open("/_ajaxphp/qualityfilter_json.php?report=excel&severityID="+severityID+"&statusId="+statusId+"&projectId="+projectId+"&clientId="+clientId+"&assignedTo="+assignedTo);
