...
class phpBasecamp {
	...
	function __construct($user, $pass, $base) {
		...
		$this->userx = $config->basecamp->username;
		$this->passwordx = $config->basecamp->password;
	...
	function companies() {
		$this->override = true;
		return $this->hook("/contacts/companies","company");
	}
	...
	function hook($url,$expected,$send = false, $overide = false) {
	...
	function request($url, $params = false) {
		if($this->password != 'OpenID'){
			//do the connect to a server thing
			$req =& new HTTP_Request($this->base . $url);
			//authorize
			($this->override) ? $req->setBasicAuth("ward.welch@nbcuni.com","beaver1"): $req->setBasicAuth($this->user, $this->pass);
			//set the headers
			...
			$req =& new HTTP_Request($this->base . $url);
			//authorize
			($this->override) ? $req->setBasicAuth("ward.welch@nbcuni.com","beaver1"): $req->setBasicAuth($this->user);
			//set the headers
			$req->addHeader("Accept", "application/xml");
			$req->addHeader("Content-Type", "application/xml");
			//if were sending stuff
	...
	function synchPeople()
	{
	}
	
	function synchProjects()
	{
	}
}
