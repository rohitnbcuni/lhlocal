<?php
	...
	if(isset($_SESSION['user_id'])) {
		...
		$select_comments = "SELECT * FROM `workorder_comments` WHERE `workorder_id`= ? AND id = ? AND deleted ='0' order by id LIMIT 0, 1";
		$comm_result = @$mysql->sqlprepare($select_comments, array($woId,$last_wid ));
		$pattern = "/(http|https|ftp|ftps)\:\/\/[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,3}(\/\S*)?/";	
		$comment_id =0;
