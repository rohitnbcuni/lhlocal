<?PHP
	...
	if(isset($_SESSION['user_id'])) {
		...
		{ 
			if(!empty($wo))
			{
				$archive_query = "UPDATE `workorders` SET `active`='1' WHERE `id` ='$wo'"; 
				@$mysql->query($archive_query);

				$draftWO_query = "select * from `workorders` WHERE `id` ='$wo'";
				
				$draftWORes = @$mysql->query($draftWO_query);
