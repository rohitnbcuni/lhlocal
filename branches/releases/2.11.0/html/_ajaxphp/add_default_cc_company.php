<?PHP
	...
	if(!empty($cc))
	{
		...
		$company_result = @$mysql->query($select_company);
		...
		if(!empty($company))
		{
			...
			$select_company_result = @$mysql->query($select_company_qry);
			 if($select_company_result->num_rows > 0)
			 {
				...
				$update_cc = "UPDATE `companies` SET `cclist`='$arrayData' WHERE `id`='$company'";
