	...
	public function createAction() 
		{			
				...
				if(isset($_GET['project_id']))
					{				
					$project_id = (int)$this->_request->getParam('project_id');
		...
		if(isset($_GET['project_id'])) 
			...
			if(isset($_GET['project_id'])) 
				...
				echo CtDisplay::getSectionEditHTMLNew($get_project_id,1);
				}
				else{	
				echo CtDisplay::getSectionEditHTML($get_project_id);	
				}}
				else
					{
		...
		if(isset($_GET['project_id']))
			{			
			echo CtDisplay::getTimelineEditHTML($get_project_id);	
			}
			else 
				{	
				...
				echo '</div>	
						...
						' . CtDisplay::getProjectStatus($get_project_id, '') . '</span>		
						<span class="dropdown">		
						...
						' . CtDisplay::getAllocationType($get_project_id) . '</select>-->
						...
						' . CtDisplay::getPrograms($get_project_id) . '</select>
						</div>				
						</div>		
						<div class="right_actions">		
		...
		if(isset($_GET['project_id'])) 
		{			
		echo CtDisplay::getOwnerRolesNewEditHTML($get_project_id);	
		}
		else
		{
		...
		if(isset($_GET['project_id'])) 
		{						
		echo CtDisplay::getResourceHTML($get_project_id);
		} 
		echo '</ul>	
		</div>';
		echo '</div>
		...
		if(isset($_GET['project_id']))
			...
			if(isset($_GET['project_id'])) 
			{	
			echo CtDisplay::getBudgetEditHTML($get_project_id);	
			}		
			if(isset($_GET['project_id'])) 
				{
				echo CtDisplay::getFinanceEditHTML($get_project_id);
				}
				else 
					{	
			 ...
			 <div id="overall_finance_total">Overall Total: <span class="cur_for">$' .@CtDisplay::getFinanceTotalEdit($get_project_id) .'</span></div>	
				</div>';
				echo '</div>	
				</form>		
			</div>	
		...
		for($u = 0; $u < sizeof($user_list); $u++) {												
			if($appr_nbc_name == $user_list[$u]['id']) {
				$selected = " SELECTED";
					...
					$selected = "";	
						...
						if(isset($_GET['project_id']))
							{
								//$get_project_id = (int) $this->_request->getParam($_GET['project_id']);
							echo CtDisplay::getApprovalsEditHTML($get_project_id);
							} else {
			...
			if(@$project_bcase[0]['desc'] != "null")
				...
				$bcaseEditor->Value = @$project_bcase[0]['desc'];
					...
					if(isset($_GET['project_id'])) 
						{				
						echo CtDisplay::projectPermissions($get_project_id);	
						} else {	
							...
							if(isset($_GET['project_id'])) {	
								echo CtDisplay::workorderPermissions($get_project_id);
								} else {		
		...
		<label>Short description of issue (one line):</label>		
				...
				<li class="label_value"> ' . CtDisplay::getRiskPermsList($get_project_id) . '</li>
				</ul>			
				<div class="add">	
				<button class="status status_flag" onClick="createRisk(\''. $get_project_id .'\', \''. $_SESSION['user_id'] .'\'); return false;"><span>Add Flag</span></button>				
				</div>			
				</div>			
				</div>	
				</form>		
				...
				<ul>' . CtDisplay::getProjectStatus($get_project_id, 'all') . '	</ul>
				</div>		
