<?PHP
	...
	if(isset($_SESSION['user_id'])) {
		 ...
		 */
		 ...
		 if(ISSET($_POST['del'])){
			if($_POST['del'] == 'lhcommentdelete'){
				$select_comments = "SELECT workorder_id FROM `workorder_comments` WHERE `id`= ? LIMIT 1";
				$comm_result = @$mysql->sqlprepare($select_comments, array($comment_id));
				if($comm_result->num_rows > 0){
					$wo_res = $comm_result->fetch_assoc();
					$woId = $wo_res['workorder_id'];
					$update_wo_comment = "UPDATE `workorder_comments`  SET `active` = '0',`deleted` = '1' WHERE id = ?";
					$r = $mysql->sqlprepare($update_wo_comment, array( $comment_id));
				
					$bc_id_query = "SELECT  `bcid`, `project_id`, `title`, `priority`,`status`,`assigned_to`,`body` FROM `workorders` WHERE `id`= ? LIMIT 1";
					$bc_id_result = $mysql->sqlprepare($bc_id_query, array($woId));
		...
		$select_comments = "SELECT workorder_id FROM `workorder_comments` WHERE `id`= ? AND deleted = '0' LIMIT 1";
		$comm_result = @$mysql->sqlprepare($select_comments, array($comment_id));
		if($comm_result->num_rows > 0){
		...
		if((!empty($comment)) && ($woId != '')){
			...
			$email_res = $mysql->sqlprepare($select_email_users, array($woId));
			$select_comments = "SELECT * FROM `workorder_comments` WHERE `id`= ? AND deleted = '0' LIMIT 1";
			$comm_result = @$mysql->sqlprepare($select_comments, array($comment_id));
			if($comm_result->num_rows > 0){
