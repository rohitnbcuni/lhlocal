...
//if (@$_GET["overtime"]) {
	...
	for($i=0;$i<count($blocks);$i++) {
		...
		if ($status==0) {
			$sql = "DELETE FROM resource_blocks WHERE userid='$user' AND daypart= ? AND datestamp='$datestamp'";
			$res = $mysql->sqlprepare($sql, array($dayblock['block']));
		} else {
			$sql = "SELECT * FROM resource_blocks WHERE userid='$user' AND daypart= ? AND datestamp='$datestamp'";
			$res = $mysql->sqlprepare($sql, array($dayblock['block']));
			if ($res->num_rows > 0) {
				if ($status<5) {
					$rb = $res ->fetch_assoc();
